...
public class Manager_ButtonHard : HandleText_Hard
{
    ...
    private void Start()
    {
        ...
        egg = 0;
        getback = true;
        SystemLevelUpHard.instance.Harder();
    }
    private void Update()
    {
        if (cout < 0) { cout = 0; }
    ...
    {
        cout = 0;
        if (text_Input.text == GameCore_Hard.string_Total) // show menu Correct
        {
            score ++;
        ...
        else  if (text_Input.text == SystemHandleEasterEgg.egg_Hard && egg == 0)  // show easter eggs
        {
            score += 100;
            egg = 1;
            text_Score.text = score.ToString();
            ...
            SystemPlayAudio.instance.PlayEffect(SystemAudioInput.instance.effect_Winner);
            Eastereggs.instance.Eastereggs_Hard(text_Input.text);
        }

        else if (text_Input.text != GameCore_Hard.string_Total && score >= 5 && getback == true) // show get back if wrong answer
        {
            getBack.SetActive(true);
            getback = false;
        }

        else if (text_Input.text != GameCore_Hard.string_Total) // show Game Over
        {
